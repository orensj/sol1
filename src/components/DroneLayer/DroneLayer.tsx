          ...
          <>
            ...
            <MapGrid
              ...
              mapItems={[
                ...
                ...drones
                  .map(droneToMapItem)
                  .filter(({ id }) => !filteredDrones.includes(id)),
              ]}
        ...
        drawer={
          <DrawerContentLayout
            ...
            listElements={
              <>
                {drones.map(
                  ({ id, name, battery, distance, launchtime, isfree }, index) => (
                    <Fragment key={id}>
                      <DroneItem
                        id={id}
                        name={name}
                        distance={distance}
                        battery={battery}
                        hidden={filteredDrones.includes(id)}
                        available={pinPosition !== undefined && isfree}
                        departureTime={launchtime}
                        onTargetClick={onTargetClick}
                        onEyeClick={onEyeClick}
                      />
                      {index + 1 !== drones.length && <hr />}
                    </Fragment>
                  )
                )}
              </>
